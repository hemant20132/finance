...
$(document).ready(function()
{
	...
	$('#cashflowtable').dataTable( {
        "processing": true,
        "serverSide": true,
        "ajax": "cashflowdelete_processing.php"
	});
 
  var table = $('#cashflowtable').DataTable();
 
	$('#cashflowtable tbody').on( 'click', 'tr', function () {
    ...
    $('#cashflowdelete').click( function () {
    
	var finyear3  = $("#cashflowtable").find(".selected td:eq(1)").text();
	var company3 = $("#cashflowtable").find(".selected td:eq(2)").text();
	
	
	if (finyear3 == "" || company3 == "")
	{
		alert("select row!!!");
		return false;
	}

	location.href = "cashflowrecorddelete.php?companycash="+company3+ "&finyearcash="+finyear3;
	});
...
<div style="">
			...
			<div  id="mainwindow" style="position:absolute;left:300px;top:100px;">
				<input type="button" style="position:absolute;left:0px;top:-20px;" class="default" id="cashflowdelete" value="delete selected row record">
				
				<form action="#"  method="POST">
				<center><h4> Delete Cash Flow Data </h4></center>
